...
export function run(filename: PathOrFileDescriptor) {
    ...
    const spelledNumberArray = arr.map(calibrationText => {
        ...
        const last = returnLastSpelledDigit(calibrationText.split('').reverse().join(''));
        return parseInt(first+last);
    });
...
function returnFirstSpelledDigit(text: string): string {
    ...
    ].filter(number => number.value !== -1).sort((a,b) => {
...
function findNumberPosition(text: string, match: string, match2: string): number {
